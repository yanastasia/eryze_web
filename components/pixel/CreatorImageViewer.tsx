...
export default function CreatorImageViewer() {
  ...
  return (
    <div className="rounded-sm border border-white bg-gradient-to-br from-[#3C165B] to-[#140E26] p-3">
      <div className="mb-3 flex items-center gap-2 text-[12px]">{"[ Open ] [ Zoom ] [ Filter ] [ Exit ]"}</div>
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
        {items.map((it, i) => (
          ...
          >
            ...
            />
            <div className="absolute bottom-1 left-1 right-1 text-[11px] bg-black/40 text-white px-1 py-0.5">{it.title}</div>
          </motion.button>
        ))}
      </div>
      <div className="mt-4 text-[12px] text-white/80">
        [<span className="inline-block align-middle" style={{ width: "160px" }}>
          <span className="inline-block h-[10px] bg-[#D073FF]" style={{ width: `${progress}%` }} />
        ...
        {selected !== null && (
          ...
          >
            ...
            >
              ...
              <motion.div className="mt-2 text-sm" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ type: "tween", duration: 0.5 }}>
